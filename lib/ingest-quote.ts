...
export type IngestQuoteSuccess = {
  ok: true;
  customerId: string;
...
export type IngestQuoteFailure = {
  ok: false;
  error: string;
  status: number;
};

export type IngestQuoteResult = IngestQuoteSuccess | IngestQuoteFailure;
...
export async function ingestQuote(body: IngestQuoteBody): Promise<IngestQuoteResult> {
  ...
  if (!DOC_PREFIX[type]) {
    return { ok: false, error: `Invalid document type "${type}"`, status: 400 };
  }
  ...
  if (lineItems.length === 0 && !(docIn.laborHours && docIn.laborHours > 0)) {
    return { ok: false, error: "Add at least one line item (or labor hours)", status: 400 };
  }
  ...
  try {
    ...
    if (!customerId) {
      return { ok: false, error: "Could not resolve customer", status: 500 };
    }
    ...
    const total = docIn.total != null ? money(Number(docIn.total)) : money(subtotal + tax);
